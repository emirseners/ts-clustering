...
def _distance_sizer_dtw(list_of_ts_objects: List['TimeSeries'], metric: str = 'sizer_dtw', distance_kwargs: dict = {}) -> Tuple[np.ndarray, List['TimeSeries']]:
    """
    Calculate pairwise pattern distances using Sizer's method, between all data sequences using Dynamic Time Warping.
    ...
    features = _construct_features_sizer(data)
    ...
    dRow = _compute_sizer_dtw_distances(features, dRow, wSlopeError, wCurvatureError)

    return dRow, list_of_ts_objects


@njit
def _compute_sizer_dtw_distances(features: List[np.ndarray], dRow: np.ndarray, wSlopeError: float, wCurvatureError: float) -> np.ndarray:
    """
